...
def mutual_information_screening(x: pd.DataFrame, y: np.ndarray) -> pd.DataFrame:
    ...
    return pd.DataFrame({"feature": x.columns, "mutual_information": scores}).sort_values("mutual_information", ascending=False)
...
def l1_logistic_screening(x: pd.DataFrame, y: np.ndarray, c: float = 0.5) -> pd.DataFrame:
    ...
    return pd.DataFrame({"feature": x.columns, "abs_coefficient": coefs}).sort_values("abs_coefficient", ascending=False)
