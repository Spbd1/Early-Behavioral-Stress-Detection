...
| Persistent alert history | MISSING | `AlertHistory` is process-local in memory only. No durable store or replay semantics. |
...
| Test coverage | PARTIAL | There are tests for synthetic workflow, HMM, ops, frontend static shape, keyword generation, geo alerting, and ingestion. Missing: live provider integration, dashboard/API schema contract with alert/report modules, persistent alert store, BSI design conformance, Docker runtime smoke, browser e2e. |
